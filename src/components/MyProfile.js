...
function MyProfile() {
    ...
    function _arrayBufferToBase64(buffer) {
      ...
      const baseString = _arrayBufferToBase64(user[0].image.data.data);

  return (
    <Grid container>
        <Grid item xl={3} lg={2} md={1}></Grid>
        <Grid item xl={6} lg={8} md={10} sm={12} xs={12}>
            <Stack direction={'column'} spacing={3} fullwidth="true"
            textAlign={"center"}>
                <Stack sx={{'display': 'flex', 'justifyContent': 'center', 'alignItems': 'center'}}>
                <img src={`data:image/png;base64,${baseString}`} height={'250px'} width={'330px'} />
                </Stack>
